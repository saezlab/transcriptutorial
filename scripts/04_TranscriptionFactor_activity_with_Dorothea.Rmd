...
and its transcriptional targets. The set of genes regulated by a specific 
...
based on the number of supporting evidence. The confidence levels range from A 
...
In this particular case, a top table results objects from limma. 
...
We have to slighly modify the format of the input files to make it suitable
to run **DoRothEA**. 
...
using the statistcs from the differential expression analysis. This will allow 
us to select the TFs whose activity varies the conditions under study. 

It is important to set the parameter `eset.filter` to `FALSE`. In this case,
we set the minimun size of regulons to five (`minsize`). I strongly recommend
to check `?viper` to decide the parameter that best fits your needs. 
...
We now compute TFs activities per every sample (with the replicates) using the 
